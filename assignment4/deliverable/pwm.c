/*
 * Kernel module for ES6, assignment 3
 * By: Stefan Grimminck & Skip Geldens
 * 
 * Use the shell script to create the device nodes, or create ones
 * with mknod yourself. We listen to minor numbers 0 to 5, and the major
 * number is created on insertion of this module (see the kernel logs).
 * 
 * The following values are accepted:
 * 0(pwm1_enable) = 0 or 1
 * 1(pwm1_freq) = 0 to 255
 * 2(pwm1_duty) = 0 to 255
 * 3(pwm2_enable) = 0 or 1
 * 4(pwm2_freq) = 0 to 255
 * 5(pwm2_duty) = 0 to 255 
 */
...
static ssize_t device_write(struct file *filp, char *buffer, size_t length, loff_t *offset) {
    ...
    switch (minor_number) {
        ...
        default:
            sprintf(msg_ptr, "This node doesn't exist");
            break;
    }
